...
public class MainController {
	...
	void handleFindClosest(ActionEvent event) {
		...
		int l = table.getSelectionModel().getSelectedItem().getParent().getValue().getLicense();
		int src = -1;
		...
		dialog.getEditor().textProperty().addListener(new ChangeListener<String>() {
			...
			public void changed(ObservableValue<? extends String> obs, String oldval, String newval) {
				if (!newval.matches("\\d*")) {
					dialog.getEditor().setText(newval.replaceAll("[^\\d]", ""));
				}
			}
		});
    ...
    private void readViols() {
    	try {
    		Scanner scnr = new Scanner(new File(System.getProperty("user.dir") + "//assets//viols.txt"));
    ...
    private void readHelp() {
    	try {
			helpdata = new String(Files.readAllBytes(Paths.get(System.getProperty("user.dir") + "//assets//help.txt")));
		} catch (Exception e) {
			Alert alert = new Alert(AlertType.ERROR);
			alert.setTitle("Exception Dialog");
			alert.setHeaderText("Exception Dialog");
				...
				if (event.getClickCount() == 2) {
					...
					try {
						if (item.isLeaf()) {
							...
							for (int vin : item.getValue().getInspection(0).getViolations()) {
