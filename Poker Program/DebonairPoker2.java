...
public class DebonairPoker2
{
  ...
  static boolean turnEnd; //variable the stores whether the turn is over or not
  static String input;
  static boolean userRaise = false; //user wishes to raise or not
  static boolean userBet = false; //user wishes to bet or not
  
  public static void main (String[ ] args) throws IOException
  {
    c = new Console (35,115); //Console size can be adjusted by changing numbers in the parameters to suit your screen size
    ...
    //Card Variables
    Hand playerHand []; //player hands
    String cardSuit [];//suits of players cards
    ...
    else if (gameChoice.equalsIgnoreCase("Rules"))
    {
      ...
      c.println ("Enter any letter key to continue the game");
      c.readLine();
      pokerRules.close();
      c.clear();
      gameChoice = "yes";
    ...
    {
      c.clear(); //Previous inputs/prompts are cleared
      ...
      while (playerMoney [0] != (numberPlayers*10000.0) && playerMoney [1] != (numberPlayers*10000.0) && playerMoney [2] != (numberPlayers*10000.0) && (playerMoney [3] != numberPlayers*10000.0))
      {
        ...
        //Ante's deducted from each player
        for (int x = 0; x < numberPlayers; x++)
        {
          playerMoney[x] -= 10; 
        }
        
        //Clears each players consoles
        for (int x = 0; x < numberPlayers; x++)
        {
          player [x].clear();
        }
        ...
        {
          turnEnd = false;//turn is not over therfore false
          userBet = false;//betis not placed therefore false
          userRaise = false;//raise is not placed therefore false
          counter = 0; //The counter for suits
          int counter2 = 0;//counter for values
          int loopCounter = 0;//counter for loops
          ...
          //While loop that performs all actions for a player during their turn
        ...
        if (foldCounter != 1)
        {
          ...
          //If there is a tie between 3 people
          else if (bestHand == playerScore [0] && bestHand == playerScore [1])
          {
            //Evaluate hands
            winner = playerHand [0].tieBreaker (bestHand, playerHand [0], playerHand [1], flopCard1, flopCard2, flopCard3); 
            c.print (playerName [winner] + " has won with a ");
            displayRank(bestHand);
            c.println (playerName [winner] + " has earned $" + pot);
